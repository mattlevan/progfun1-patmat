...
object Huffman {
   ...
   * leaves.
   */
  sealed abstract class CodeTree { val weight: Int }
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def help(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] =
      chars match {
        case Nil => acc
        case h :: t =>
          if (acc.exists(p => p._1 == h)) help(t, acc)
          else help(t, count(h, 0, chars) :: acc)
      }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def getChar(t: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
      t match {
        case Fork(l, r, _, _) =>
          bits match {
            case 0 :: t => getChar(l, t)
            case 1 :: t => getChar(r, t)
            case _      => throw new Error("Invalid bit.")
          }
        case Leaf(c, _) => (c, bits)
      }
    }

    def collect(bits: List[Bit], xs: List[Char]): List[Char] =
      bits match {
        case Nil    => xs
        case _ =>
          val x = getChar(tree, bits)
          collect(x._2, xs :+ x._1)
      }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def parseText(text: List[Char], bits: List[Bit]): List[Bit] =
      text match {
        case Nil    => bits
